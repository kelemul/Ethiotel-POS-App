...
def _install_stubs():
    ...
    fetchers = types.ModuleType("ethiotel_pos.api.item_fetchers")
    fetchers.ItemDetailAggregator = object
    sys.modules["ethiotel_pos.api.item_fetchers"] = fetchers

    utils = types.ModuleType("ethiotel_pos.api.utils")
    ...
    sys.modules["ethiotel_pos.api.utils"] = utils

    barcode = types.ModuleType("ethiotel_pos.api.item_processing.barcode")
    barcode.search_serial_or_batch_or_barcode_number = lambda *args, **kwargs: None
    sys.modules["ethiotel_pos.api.item_processing.barcode"] = barcode

    details = types.ModuleType("ethiotel_pos.api.item_processing.details")
    details.get_items_details = lambda *args, **kwargs: []
    sys.modules["ethiotel_pos.api.item_processing.details"] = details


def _load_module():
    module_name = "test_item_search_serialization_target"
    file_path = REPO_ROOT / "ethiotel_pos" / "ethiotel_pos" / "api" / "item_processing" / "search.py"
