...
from ethiotel_pos import __version__ as POS_AWESOME_APP_VERSION
...
from ethiotel_pos.utils import get_build_version
...
def _get_git_commit_info(app_name: str = "ethiotel_pos") -> Dict[str, Any]:
...
def get_build_info() -> Dict[str, Any]:
    ...
    data.update(_get_git_commit_info("ethiotel_pos"))
    return data
...
def get_remote_update_info() -> Dict[str, Any]:
    data: Dict[str, Any] = {"build_version": get_build_version(), **_get_update_metadata()}
    base = _get_git_commit_info("ethiotel_pos")
    if base:
        data.update(base)

    try:
        app_path = frappe.get_app_path("ethiotel_pos")
    except Exception:
        return data
...
def get_available_languages():
    ...
    try:
        translations_path = frappe.get_app_path("ethiotel_pos", "translations")
        if os.path.exists(translations_path):
...
def get_language_info(lang_code):
    ...
    try:
        ...
        translations_path = frappe.get_app_path("ethiotel_pos", "translations", f"{lang_code}.csv")
