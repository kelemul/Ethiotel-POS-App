...
sys.path.insert(
    0,
    str(REPO_ROOT / "ethiotel_pos" / "ethiotel_pos" / "api" / "test_support"),
)
...
def _install_stubs():
    ...
    invoice_utils_module = types.ModuleType("ethiotel_pos.api.invoice_processing.utils")
    invoice_utils_module.get_price_list_currency = lambda price_list: "PKR"
    sys.modules["ethiotel_pos.api.invoice_processing.utils"] = invoice_utils_module

    api_utils_module = types.ModuleType("ethiotel_pos.api.utils")
    ...
    sys.modules["ethiotel_pos.api.utils"] = api_utils_module
    sys.modules["ethiotel_pos.api.offline_sync.common"] = load_offline_sync_common()


def _load_module():
    module_name = "test_offline_sync_bootstrap_target"
    file_path = REPO_ROOT / "ethiotel_pos" / "ethiotel_pos" / "api" / "offline_sync" / "bootstrap.py"
...
class TestOfflineSyncBootstrap(unittest.TestCase):
    ...
    def test_sync_bootstrap_config_returns_bootstrap_and_price_list_meta_changes(self):
        ...
        self.assertEqual(response["changes"][1]["data"]["selected_price_list"], "Retail")
