...
def _install_dependency_stubs():
    item_fetchers_module = types.ModuleType("ethiotel_pos.api.item_fetchers")
    item_fetchers_module.ItemDetailAggregator = object
    item_fetchers_module.get_batches = lambda *args, **kwargs: []
    sys.modules["ethiotel_pos.api.item_fetchers"] = item_fetchers_module

    stock_module = types.ModuleType("ethiotel_pos.api.item_processing.stock")
    stock_module.get_stock_availability = lambda *args, **kwargs: 0
    sys.modules["ethiotel_pos.api.item_processing.stock"] = stock_module
...
def _install_package_stubs():
    package_paths = {
        ...
        "ethiotel_pos.api.item_processing": (
            REPO_ROOT / "ethiotel_pos" / "ethiotel_pos" / "api" / "item_processing"
        ),
    }
...
def _load_module():
    module_name = "ethiotel_pos.api.item_processing.details"
    file_path = REPO_ROOT / "ethiotel_pos" / "ethiotel_pos" / "api" / "item_processing" / "details.py"
