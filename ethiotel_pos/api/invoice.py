...
from ethiotel_pos.api.utilities import get_company_domain  # Updated import
from ethiotel_pos.api.payments import get_ethiotel_pos_credit_redeem_remark
from ethiotel_pos.ethio_telecom_pos_app.doctype.delivery_charges.delivery_charges import (
    get_applicable_delivery_charges,
)
from ethiotel_pos.ethio_telecom_pos_app.doctype.pos_coupon.pos_coupon import update_coupon_code_count
...
def on_cancel(doc, method):
    cancel_ethiotel_pos_credit_journal_entries(doc)
    restore_ethiotel_pos_gift_card_redemptions(doc)
...
def cancel_ethiotel_pos_credit_journal_entries(doc):
    remark = get_ethiotel_pos_credit_redeem_remark(doc.name)
    ...
    for journal_entry in linked_journal_entries:
        ...
        try:
            ...
            frappe.log_error(
                ...
                "Ethiotel_POS Credit Journal Cancellation Error",
            )
            frappe.throw(
                _(
...
def restore_ethiotel_pos_gift_card_redemptions(doc):
    try:
        from ethiotel_pos.api.gift_cards import restore_invoice_gift_card_redemptions

        restore_invoice_gift_card_redemptions(doc)
    except Exception:
        frappe.log_error(
            frappe.get_traceback(),
            "Ethiotel_POS Gift Card Restoration Error",
        )
        frappe.throw(
            _(
