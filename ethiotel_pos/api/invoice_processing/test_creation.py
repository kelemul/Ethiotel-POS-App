...
def _install_dependency_stubs():
    ...
    processing_utils = types.ModuleType("ethiotel_pos.api.invoice_processing.utils")
    ...
    sys.modules["ethiotel_pos.api.invoice_processing.utils"] = processing_utils

    stock_module = types.ModuleType("ethiotel_pos.api.invoice_processing.stock")
    ...
    sys.modules["ethiotel_pos.api.invoice_processing.stock"] = stock_module

    payment_utils_module = types.ModuleType("ethiotel_pos.api.payment_processing.utils")
    payment_utils_module.get_bank_cash_account = lambda *_args, **_kwargs: None
    sys.modules["ethiotel_pos.api.payment_processing.utils"] = payment_utils_module

    utilities_module = types.ModuleType("ethiotel_pos.api.utilities")
    ...
    sys.modules["ethiotel_pos.api.utilities"] = utilities_module

    payments_module = types.ModuleType("ethiotel_pos.api.payments")
    payments_module.redeeming_customer_credit = lambda *_args, **_kwargs: None
    sys.modules["ethiotel_pos.api.payments"] = payments_module
...
def _install_package_stubs():
    package_paths = {
        ...
        "ethiotel_pos.api.invoice_processing": (
            REPO_ROOT / "ethiotel_pos" / "ethiotel_pos" / "api" / "invoice_processing"
        ),
    }
...
def _load_module():
    module_name = "ethiotel_pos.api.invoice_processing.creation"
    file_path = REPO_ROOT / "ethiotel_pos" / "ethiotel_pos" / "api" / "invoice_processing" / "creation.py"
...
class TestPostSubmitPaymentProcessing(unittest.TestCase):
    ...
    def test_run_post_submit_payments_passes_created_receive_entries_to_change_entry_creation(self):
        ...
        payment_module_name = "ethiotel_pos.api.invoice_processing.payment"
    ...
    def test_apply_invoice_gift_card_settlement_delegates_before_submit(self):
        ...
        gift_card_module_name = "ethiotel_pos.api.gift_cards"
        gift_card_calls = []
