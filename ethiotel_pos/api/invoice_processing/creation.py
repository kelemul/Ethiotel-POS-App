...
from ethiotel_pos.api.invoice_processing.utils import (
...
from ethiotel_pos.api.invoice_processing.stock import (
...
from ethiotel_pos.api.payment_processing.utils import get_bank_cash_account as get_bank_account
from ethiotel_pos.api.utilities import ensure_child_doctype, set_batch_nos_for_bundels
from ethiotel_pos.api.payments import redeeming_customer_credit
from ethiotel_pos.api.idempotency import (
...
def _apply_invoice_gift_card_settlement(invoice_doc, data):
    from ethiotel_pos.api.gift_cards import apply_invoice_gift_card_redemptions
...
def _run_post_submit_payments(invoice_doc, data, is_payment_entry, total_cash, cash_account, payments):
    from ethiotel_pos.api.invoice_processing.payment import _create_change_payment_entries
...
def update_invoice(data):
    ...
    if (data.get("is_return") or invoice_doc.is_return) and invoice_doc.get("return_against"):
        ...
        from ethiotel_pos.api.invoice_processing.returns import validate_return_items
