...
INVOICE_API_PATH = REPO_ROOT / "ethiotel_pos" / "ethiotel_pos" / "api" / "invoice.py"
INVOICES_API_PATH = REPO_ROOT / "ethiotel_pos" / "ethiotel_pos" / "api" / "invoices.py"
...
def _install_invoice_api_stubs():
    ...
    delivery_charges_module = types.ModuleType(
        "ethiotel_pos.ethio_telecom_pos_app.doctype.delivery_charges.delivery_charges"
    )
    pos_coupon_module = types.ModuleType("ethiotel_pos.ethio_telecom_pos_app.doctype.pos_coupon.pos_coupon")
    ...
    payments_module.get_ethiotel_pos_credit_redeem_remark = lambda invoice_name: f"redeem {invoice_name}"
    ...
    sys.modules["ethiotel_pos.ethio_telecom_pos_app.doctype.pos_coupon.pos_coupon"] = pos_coupon_module

    return state


def _load_invoice_api_module():
    module_name = "ethiotel_pos.api.invoice"
...
class TestInvoiceCancelHooks(unittest.TestCase):
    def test_sales_invoice_cancel_hook_restores_gift_cards(self):
        ...
        self.assertIn('"on_cancel": "ethiotel_pos.api.invoice.on_cancel"', hooks)
    ...
    def test_cancel_hook_deletes_matching_submission_ledger_entries(self):
        ...
        invoice_api.cancel_ethiotel_pos_credit_journal_entries = lambda doc: called.update(credit=True)
        invoice_api.restore_ethiotel_pos_gift_card_redemptions = lambda doc: called.update(gift_cards=True)
