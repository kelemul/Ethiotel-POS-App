...
def _install_frappe_stub():
    ...
    invoice_processing_utils = types.ModuleType("ethiotel_pos.api.invoice_processing.utils")
    ...
    sys.modules["ethiotel_pos.api.invoice_processing.utils"] = invoice_processing_utils

    invoice_processing_stock = types.ModuleType("ethiotel_pos.api.invoice_processing.stock")
    ...
    sys.modules["ethiotel_pos.api.invoice_processing.stock"] = invoice_processing_stock

    invoice_processing_creation = types.ModuleType("ethiotel_pos.api.invoice_processing.creation")
    ...
    sys.modules["ethiotel_pos.api.invoice_processing.creation"] = invoice_processing_creation

    invoice_processing_returns = types.ModuleType("ethiotel_pos.api.invoice_processing.returns")
    ...
    sys.modules["ethiotel_pos.api.invoice_processing.returns"] = invoice_processing_returns

    invoice_processing_payment = types.ModuleType("ethiotel_pos.api.invoice_processing.payment")
    invoice_processing_payment._create_change_payment_entries = lambda *args, **kwargs: None
    sys.modules["ethiotel_pos.api.invoice_processing.payment"] = invoice_processing_payment

    invoice_processing_data = types.ModuleType("ethiotel_pos.api.invoice_processing.data")
    invoice_processing_data.get_last_invoice_rates = lambda *args, **kwargs: []
    sys.modules["ethiotel_pos.api.invoice_processing.data"] = invoice_processing_data

    utils_module = types.ModuleType("ethiotel_pos.api.utils")
    utils_module.log_perf_event = lambda *args, **kwargs: None
    sys.modules["ethiotel_pos.api.utils"] = utils_module

    erpnext_compat_module = types.ModuleType("ethiotel_pos.api.erpnext_compat")
    erpnext_compat_module.resolve_make_sales_invoice_from_order = lambda: erpnext_sales_order.make_sales_invoice
    sys.modules["ethiotel_pos.api.erpnext_compat"] = erpnext_compat_module


def _load_invoices_module():
    module_name = "ethiotel_pos.api.invoices"
    file_path = REPO_ROOT / "ethiotel_pos" / "ethiotel_pos" / "api" / "invoices.py"
