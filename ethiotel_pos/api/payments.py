...
from ethiotel_pos.api.erpnext_compat import resolve_get_party_bank_account
from erpnext.accounts.doctype.payment_request.payment_request import (
    get_dummy_message,
    get_existing_payment_request_amount,
)
from ethiotel_pos.api.utilities import ensure_child_doctype

def get_party_bank_account(*args, **kwargs):
    return resolve_get_party_bank_account()(*args, **kwargs)


def get_ethiotel_pos_credit_redeem_remark(invoice_name):
    ...
    if data.get("redeemed_customer_credit"):
        ...
        for row in data.get("customer_credit_dict"):
            if row["type"] == "Invoice" and row["credit_to_redeem"]:
                ...
                jv_doc.user_remark = get_ethiotel_pos_credit_redeem_remark(invoice_doc.name)
                jv_doc.set_missing_values()
                try:
                    jv_doc.save()
                    jv_doc.submit()
                except Exception as e:
                    frappe.log_error(frappe.get_traceback(), "Ethiotel_POS JV Error")
