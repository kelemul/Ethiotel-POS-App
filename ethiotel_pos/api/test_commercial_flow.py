...
def _install_stubs():
    module_names = [
        "frappe",
        "frappe.utils",
        "ethiotel_pos.api.invoices",
        "ethiotel_pos.api.quotations",
        "ethiotel_pos.api.sales_orders",
        "ethiotel_pos.api.erpnext_compat",
    ...
    invoices_module = types.ModuleType("ethiotel_pos.api.invoices")
    invoices_module.get_draft_invoices = lambda **kwargs: []
    sys.modules["ethiotel_pos.api.invoices"] = invoices_module

    quotations_module = types.ModuleType("ethiotel_pos.api.quotations")
    ...
    sys.modules["ethiotel_pos.api.quotations"] = quotations_module

    sales_orders_module = types.ModuleType("ethiotel_pos.api.sales_orders")
    sales_orders_module.search_orders = lambda **kwargs: []
    sys.modules["ethiotel_pos.api.sales_orders"] = sales_orders_module
...
def _load_module():
    module_name = "ethiotel_pos.api.commercial_flow"
    file_path = REPO_ROOT / "ethiotel_pos" / "ethiotel_pos" / "api" / "commercial_flow.py"
...
class TestCommercialFlowApi(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        sys.modules.pop("ethiotel_pos.api.commercial_flow", None)
