...
def _install_framework_stubs():
    ...
    mpesa_module = types.ModuleType("ethiotel_pos.api.m_pesa")
    mpesa_module.submit_mpesa_payment = lambda *args, **kwargs: None
    sys.modules["ethiotel_pos.api.m_pesa"] = mpesa_module


def _install_package_stubs():
    package_paths = {
        "ethiotel_pos": REPO_ROOT / "ethiotel_pos",
        "ethiotel_pos": REPO_ROOT / "ethiotel_pos" / "ethiotel_pos",
        "ethiotel_pos.api": REPO_ROOT / "ethiotel_pos" / "ethiotel_pos" / "api",
        "ethiotel_pos.api.payment_processing": (
            REPO_ROOT / "ethiotel_pos" / "ethiotel_pos" / "api" / "payment_processing"
        ),
    }
...
class TestPosPaymentProcessing(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        sys.modules.pop("ethiotel_pos.api.idempotency", None)
        payment_processing_dir = REPO_ROOT / "ethiotel_pos" / "ethiotel_pos" / "api" / "payment_processing"
        _load_module(
            "ethiotel_pos.api.payment_processing.utils",
            payment_processing_dir / "utils.py",
        )
        cls.creation = _load_module(
            "ethiotel_pos.api.payment_processing.creation",
            payment_processing_dir / "creation.py",
        )
        cls.data = _load_module(
            "ethiotel_pos.api.payment_processing.data",
            payment_processing_dir / "data.py",
        )
        cls.processor = _load_module(
            "ethiotel_pos.api.payment_processing.processor",
            payment_processing_dir / "processor.py",
        )
        cls.reconciliation = _load_module(
            "ethiotel_pos.api.payment_processing.reconciliation",
    ...
    @patch("ethiotel_pos.api.payment_processing.processor.create_payment_entry")
    @patch("ethiotel_pos.api.payment_processing.processor.frappe")
    ...
    @patch("ethiotel_pos.api.payment_processing.processor.find_payment_entries_by_client_request_id")
    @patch("ethiotel_pos.api.payment_processing.processor.create_payment_entry")
    @patch("ethiotel_pos.api.payment_processing.processor.frappe")
    ...
    @patch("ethiotel_pos.api.payment_processing.processor.get_account_currency")
    @patch("ethiotel_pos.api.payment_processing.processor.create_payment_entry")
    @patch("ethiotel_pos.api.payment_processing.processor.frappe")
    ...
    @patch("ethiotel_pos.api.payment_processing.data.get_advance_payment_entries_for_regional")
    @patch("ethiotel_pos.api.payment_processing.data.get_party_account")
    @patch("ethiotel_pos.api.payment_processing.data.frappe")
    ...
    @patch(
        "ethiotel_pos.api.payment_processing.data.get_erpnext_outstanding_invoices",
    ...
    @patch("ethiotel_pos.api.payment_processing.reconciliation.reconcile_against_document")
    @patch("ethiotel_pos.api.payment_processing.reconciliation.frappe")
    @patch("ethiotel_pos.api.payment_processing.reconciliation.get_unallocated_payments")
    @patch("ethiotel_pos.api.payment_processing.reconciliation.get_outstanding_invoices")
